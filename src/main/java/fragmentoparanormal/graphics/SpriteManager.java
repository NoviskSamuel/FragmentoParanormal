package fragmentoparanormal.graphics;
...
public class SpriteManager {
    ...
    /**
     * Carrega imagem com cache. Retorna null se não encontrar (sem lançar exceção).
     */
    ...
    /** Define a imagem de um ImageView. Se imagem não existir, mantém a atual. */
    ...
    /**
     * Animação idle: leve pulsação em loop.
     * @return a animação para poder parar externamente (st.stop())
     */
    ...
    /**
     * Shake horizontal rápido (ataque).
     * Para animações anteriores antes de iniciar.
     */
    ...
    public void exibirJumpscare(ImageView iv, String imagemPath, Runnable aoTerminar) {
        ...
        // Segurança: usa imagem de fallback se não encontrar
        Image img = getImagem(imagemPath);
        if (img == null) {
            // Sem imagem de jumpscare: pula direto para callback
        ...
        fadeIn.setOnFinished(e1 -> {
            // 2) Scale down para tamanho normal (efeito zoom)
    ...
    /** Libera cache de imagens. Chame ao trocar de missão para liberar memória. */
