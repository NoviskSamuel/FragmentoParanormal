...
public class InventarioDAO {
    ...
    public void adicionarItem(int personagemId, int itemId) throws SQLException {
        String sql = "INSERT INTO inventario (personagem_id, item_id) VALUES (?,?) ON CONFLICT DO NOTHING";
    ...
    public void removerItem(int personagemId, int itemId) throws SQLException {
        String sql = "DELETE FROM inventario WHERE personagem_id = ? AND item_id = ?";
        try (PreparedStatement ps = ConexaoDB.getConexao().prepareStatement(sql)) {
            ps.setInt(1, personagemId);
            ps.setInt(2, itemId);
            ps.executeUpdate();
    ...
    public void limparInventario(int personagemId) throws SQLException {
        String sql = "DELETE FROM inventario WHERE personagem_id = ?";
        try (PreparedStatement ps = ConexaoDB.getConexao().prepareStatement(sql)) {
            ps.setInt(1, personagemId);
            ps.executeUpdate();
        }
